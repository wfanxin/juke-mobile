<template>
	<view class="content">
		<view class="no-data" v-if="invite_list.length === 0 && loading">
			<image class="no-image" src="../../static/no_data.png" mode=""></image>
			<view class="no-text">暂无数据</view>
		</view>
		<view class="panel-wrap" v-else>
			<view class="panel">
				<view class="panel-box" v-for="item in invite_list" :key="item.id">
...
<script>
	export default {
		...
		methods: {
			getInviteMemberList() {
				this.loading = false
				this.$server.requestGet('user/getInviteMemberList', {}).then((data) => {
					this.invite_list = data.data.data
					this.loading = true
				}).catch(() => {
					this.loading = true
				})
